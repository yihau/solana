...
impl NetworkTopology {
    ...
    fn new_random(max_partitions: usize, max_packet_drop: u8, max_packet_delay: u32) -> Self {
        let mut rng = rng();
        let num_partitions = rng.random_range(0..max_partitions + 1);
        ...
        for i in 0..num_partitions {
            let partition = if i == num_partitions - 1 {
                ...
                rng.random_range(0..100 - used_partition - num_partitions + i)
            };
        ...
        for i in 0..partitions.len() - 1 {
            for j in i + 1..partitions.len() {
                let drop_config = if max_packet_drop > 0 {
                    let packet_drop = rng.random_range(0..max_packet_drop + 1);
                ...
                let config = if max_packet_delay > 0 {
                    let packet_delay = rng.random_range(0..max_packet_delay + 1);
