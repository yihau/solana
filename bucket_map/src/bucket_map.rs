...
mod tests {
    ...
    fn hashmap_compare() {
        ...
        for mut use_batch_insert in [true, false] {
            ...
            let gen_rand_value = || {
                let count = rng().gen_range(0..max_slot_list_len);
                let v = (0..count)
                    .map(|x| (x as usize, x as usize /*rng().gen::<usize>()*/))
                    .collect::<Vec<_>>();
                let range = rng().gen_range(0..100);
                ...
                let rc = if range < 50 {
                    ...
                    rng().gen_range(0..MAX_LEGAL_REFCOUNT)
                };
            ...
            let get_key = || {
                ...
                let len = keys.len();
                Some(keys.remove(rng().gen_range(0..len)))
            };
            let return_key = |key| {
                let mut keys = all_keys.lock().unwrap();
                keys.push(key);
            ...
            for i in 0..10000 {
                ...
                if initial > 0 || rng().gen_range(0..5) == 0 {
                    // insert
                    let mut to_add = 1;
                    if initial > 1 && use_batch_insert {
                        to_add = rng().gen_range(1..(initial / 4).max(2));
                        initial -= to_add;
                    }
                    ...
                    maps.iter().for_each(|map| {
                        ...
                        if batch_insert_now {
                            ...
                            if batch_additions.len() > 1 && rng().gen_range(0..2) == 0 {
                                // insert a duplicate sometimes
                                let item_to_duplicate =
                                    rng().gen_range(0..batch_additions.len());
                ...
                if rng().gen_range(0..10) == 0 {
                    // add/unref
                    if let Some(k) = get_key() {
                        let mut inc = rng().gen_range(0..2) == 0;
                        let mut hm = hash_map.write().unwrap();
